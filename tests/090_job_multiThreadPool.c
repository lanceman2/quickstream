...
int main(void) {
    ...
    ASSERT(NumThreadPools);
    ASSERT(NumBlocks > 2);
    ASSERT(NumThreadPools <= NumBlocks);
    ASSERT(NumJobsPerBlock >= 1);

    uint32_t BlocksPerThreadPool = NumBlocks/NumThreadPools;

    // Array of pointers to thread pools:
    struct QsThreadPool *tps[NumThreadPools];
    uint32_t threadPoolIndex = 0;
    uint32_t numThreadPools = 0;


    for(uint32_t i = 0; i < NumBlocks;) {

        if(threadPoolIndex < NumThreadPools) {
            tps[threadPoolIndex] = qsGraph_createThreadPool(g, 4, 0/*name*/);
            ++numThreadPools;
        }

        uint32_t num = i + BlocksPerThreadPool;
        for(; i< num && i < NumBlocks; ++i)
            Block_init(g, blocks + i, tps[threadPoolIndex]);

        ++threadPoolIndex;
     }


    // Make the rest of the thread pools, if any.
    for(;numThreadPools < NumThreadPools; ++numThreadPools)
        tps[numThreadPools] = qsGraph_createThreadPool(g, 4, 0/*name*/);
...
#ifndef NO_QUEUE
            // No job locking needed because we have a thread pool halt
...
// Use these and run "htop" and see CPUs in action.
//#define WorkCount  900000
//#define PlusCount  910000
